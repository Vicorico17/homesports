...
export function PlayoffBracket({ matches }: { matches: BracketMatch[] }) {
  ...
  return <div className="playoff-bracket"><p className="bracket-hint">Follow each round from left to right · All times UTC</p>{layoutBracket(matches).map(lane => <section className="bracket-lane" key={lane.name} aria-label={lane.name}><h3>{lane.name}</h3><div className="bracket-board" tabIndex={0} role="region" aria-label={`${lane.name}, scroll to see all rounds`}><div style={{ width: lane.width, minWidth: lane.width }}><div className="bracket-headers">{lane.rounds.map((round, index) => <h4 key={index}>{roundLabel(round[0])}</h4>)}</div><div className="bracket-canvas" style={{ height: lane.height }}><svg className="bracket-connectors" width={lane.width} height={lane.height} aria-hidden="true">{lane.edges.map(edge => <path key={edge.key} strokeDasharray={edge.loser ? "4 4" : undefined} d={`M ${edge.from.x + 256} ${edge.from.y} H ${edge.to.x - 16} V ${edge.to.y} H ${edge.to.x}`} />)}</svg>{lane.rounds.flat().map(match => {
    ...
    return <article className={`bracket-match${match.status === "running" ? " is-live" : ""}`} key={match.id} style={{ left: position.x, top: position.y - 50 }} aria-label={match.name}><div className="bracket-meta"><span>{labels.get(match.id)}</span><span>{match.status === "running" ? "● LIVE" : match.status === "finished" ? "Finished" : match.status === "canceled" ? "Canceled" : time}</span></div>{[0, 1].map(index => {
