...
export function Matchboard({ matches: initialMatches, demo }: { matches: Match[]; demo: boolean }) {
  ...
  return <div className="shell">
    ...
    <section className="board"><div className="board-heading"><h2>{tab === "running" ? "Live now" : tab === "upcoming" ? "Upcoming matches" : tab === "finished" ? "Recent results" : "The match board"}</h2><small>{list.length} matches · Updates every minute</small></div>{list.length ? <div className="cards">{list.map((match) => <article className={`match ${match.status}`} key={match.id}><div className="meta"><span className="status">{timeLabel(match.beginAt, match.status)}</span><span>{match.league} · BO{match.bestOf}</span></div><div className="teams">{match.opponents.slice(0, 2).map((team, teamIndex) => <div className="team" key={team.name}>{team.imageUrl ? <img src={team.imageUrl} alt="" /> : <span className="crest">{team.name.slice(0, 1)}</span>}<b>{team.name}</b>{(match.status === "upcoming" || match.status === "running") && match.odds?.[0] && <span className="team-odd">{(teamIndex === 0 ? match.odds[0].home : match.odds[0].away)?.toFixed(2) ?? "—"}</span>}{match.status !== "upcoming" && <strong>{team.score ?? "—"}</strong>}</div>)}{match.status === "upcoming" && <div className="start-countdown">STARTS IN <b>{startsIn(match.beginAt)}</b></div>}</div><footer><span><StarRating value={match.importance} /> <small>{match.importanceReason}</small></span><span>{match.tournament || match.serie}</span></footer></article>)}</div> : <div className="empty">No matches meet this filter.</div>}</section>
  </div>;
}
