...
export function Matchboard({ matches: initialMatches, demo, source, sourceStatus, updatedAt, view = "all" }: MatchFeed & { view?: MatchStatus | "all" }) {
  ...
  return <div className="shell">
    <header><Link className="brand" href="/"><span>HOME</span>SPORTS</Link><p>League of Legends esports, ranked by what matters.</p><Link className="preselect-nav-link" href="/preselect-twitch-extension">PRESELECT <span>TWITCH EXTENSION</span></Link><div className="pulse"><b /> {feed.sourceStatus === "healthy" ? `LIVE DATA · ${new Date(feed.updatedAt).toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })}` : "DEGRADED DATA"}</div></header>
