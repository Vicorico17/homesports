...
const tabs: { label: string; status: BoardView }[] = [
  { label: "All matches", status: "all" },
  { label: "Following", status: "following" },
  { label: "Live", status: "running" },
...
function timeLabel(value: string, status: MatchStatus) {
  ...
  if (!value || !Number.isFinite(date.getTime())) return "DATE TBD";
...
function startsIn(value: string) {
  ...
  if (!value || !Number.isFinite(timestamp)) return "TBD";
...
export function Matchboard({ matches: initialMatches, demo, source, sourceStatus, updatedAt, view = "all" }: MatchFeed & { view?: MatchStatus | "all" }) {
  ...
  const list = useMemo(() => matches.filter((match) => (activeView === "all" || activeView === "following" || match.status === activeView) && (activeView !== "following" || match.opponents.some((team) => team.id && followedTeams.includes(String(team.id)))) && (league === "all" || match.league === league) && match.importance >= minimum && (!query.trim() || [match.name, match.league, match.tournament, match.serie, ...match.opponents.map(team => team.name)].some(value => value.toLowerCase().includes(query.trim().toLowerCase())))), [matches, activeView, followedTeams, league, minimum, query]);
  const nextFollowed = activeView === "following" ? list.find(match => match.status === "running" || match.status === "upcoming") : undefined;

  useEffect(() => {
    const refreshFollowedTeams = () => setFollowedTeams(readFollowedTeams());
    refreshFollowedTeams();
    window.addEventListener(FOLLOWED_TEAMS_EVENT, refreshFollowedTeams);
    window.addEventListener("storage", refreshFollowedTeams);
    return () => { window.removeEventListener(FOLLOWED_TEAMS_EVENT, refreshFollowedTeams); window.removeEventListener("storage", refreshFollowedTeams); };
  }, []);
  ...
  return <div className="shell">
    <header><Link className="brand" href="/"><span>HOME</span>SPORTS</Link><p>League of Legends esports, ranked by what matters.</p><nav className="site-links" aria-label="Site"><Link href="/worlds">Worlds 2026</Link><Link href="/results">Results</Link><Link href="/calendar">Calendar</Link><Link href="/preselect-twitch-extension">Preselect</Link></nav><div className="pulse"><b /> {feed.sourceStatus === "healthy" ? `LIVE DATA · ${new Date(feed.updatedAt).toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })}` : "DEGRADED DATA"}</div></header>
