...
export function Matchboard({ matches: initialMatches, demo }: { matches: Match[]; demo: boolean }) {
  ...
  return <div className="shell">
    <header><a className="brand" href="/"><span>HOME</span>SPORTS</a><p>League of Legends esports, ranked by what matters.</p><a className="calendar-link" href="/calendar">CALENDAR</a><div className="pulse"><b /> LIVE DATA</div></header>
    ...
    <section className="board"><div className="board-heading"><h2>{tab === "running" ? "Live now" : tab === "upcoming" ? "Upcoming matches" : tab === "finished" ? "Recent results" : "The match board"}</h2><small>{list.length} matches · Updates every minute</small></div>{list.length ? <div className="cards">{list.map((match) => <article className={`match ${match.status}`} key={match.id}><div className="meta"><span className="status">{timeLabel(match.beginAt, match.status)}</span><span>{match.league} · BO{match.bestOf}</span></div><div className="teams">{match.opponents.slice(0, 2).map((team) => <div className="team" key={team.name}>{team.imageUrl ? <img src={team.imageUrl} alt="" /> : <span className="crest">{team.name.slice(0, 1)}</span>}<b>{team.name}</b>{match.status !== "upcoming" && <strong>{team.score ?? "—"}</strong>}</div>)}{match.status === "upcoming" && <div className="start-countdown">STARTS IN <b>{startsIn(match.beginAt)}</b></div>}</div><footer><span><StarRating value={match.importance} /> <small>{match.importanceReason}</small></span><span>{match.tournament || match.serie}</span></footer>{(match.status === "upcoming" || match.status === "running") && match.opponents.length === 2 && <div className="pick-row odds-row"><span>{match.status === "running" ? "LIVE AVG" : "AVG ODDS"}</span>{match.odds?.[0] ? <><span className={`odds-book ${match.odds[0].home !== undefined && match.odds[0].away !== undefined && match.odds[0].home < match.odds[0].away ? "favorite" : ""}`}><small>{match.opponents[0].name}</small><b>{match.odds[0].home?.toFixed(2) ?? "—"}</b>{match.odds[0].home !== undefined && match.odds[0].away !== undefined && match.odds[0].home < match.odds[0].away && <i>FAV</i>}</span><span className={`odds-book ${match.odds[0].home !== undefined && match.odds[0].away !== undefined && match.odds[0].away < match.odds[0].home ? "favorite" : ""}`}><small>{match.opponents[1].name}</small><b>{match.odds[0].away?.toFixed(2) ?? "—"}</b>{match.odds[0].home !== undefined && match.odds[0].away !== undefined && match.odds[0].away < match.odds[0].home && <i>FAV</i>}</span></> : <small>Odds unavailable</small>}</div>}</article>)}</div> : <div className="empty">No matches meet this filter.</div>}</section>
  </div>;
}
