...
export default async function CompetitionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="competition-page">
    ...
    <section className="competition-section"><div className="section-heading"><h2>Bracket</h2>{bracket.length ? <Link className="bracket-link" href={`/bracket/${id}`}>Open full bracket ↗</Link> : null}</div>{rounds.length ? <div className="bracket-board">{rounds.map(([round, matches], roundIndex) => <div className="bracket-round" key={round}><h3>{round}</h3><div className="bracket-round-matches" style={{ gridTemplateRows: `repeat(${bracketSlots}, minmax(82px, auto))` }}>{matches.map((match, matchIndex) => { const scores = match.results?.map((result) => result.score ?? "—") ?? []; const span = 2 ** roundIndex; return <div className="bracket-match" style={{ gridRow: `${matchIndex * span + 1} / span ${span}` }} key={match.id}>{[0, 1].map((index) => <div className="bracket-team" key={index}>{match.opponents?.[index]?.opponent?.image_url ? <img src={match.opponents[index].opponent.image_url} alt="" /> : <i>{bracketTeam(match, index).slice(0, 1)}</i>}<span>{bracketTeam(match, index)}</span><b>{scores[index] ?? "—"}</b></div>)}</div>; })}</div></div>)}</div> : <p className="empty">No bracket is available for this competition yet.</p>}</section>
  </main>;
}
