...
async function getVerifiedRoster(teamName: string, pandaPlayers: Player[]) {
  try {
    ...
    if (!response.ok) return await getRenderedRoster(teamName, pandaPlayers);
...
export default async function TeamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  teamMatches = [...new Map([...teamMatches, ...localMatches.filter(match => match.status !== "finished").map(match => ({ id: match.id, status: match.status === "upcoming" ? "not_started" : match.status, begin_at: match.beginAt, opponents: match.opponents.map(team => ({ opponent: { id: team.id, name: team.name, image_url: team.imageUrl } })), results: match.opponents.map(team => ({ team_id: team.id, score: team.score })), league: { name: match.league, image_url: match.leagueImageUrl }, tournament: { id: match.tournamentId, name: match.tournament, image_url: match.tournamentImageUrl } }))].map(match => [match.id, match])).values()];
  ...
  const displayedRoster = rosterVerified ? starters : declaredStarters;
  ...
  const recent = teamMatches.filter((match) => match.status === "finished"); const form = recent.slice(0, 5).map((match) => { const current = match.results?.find((result) => result.team_id === Number(id)); const other = match.results?.find((result) => result.team_id !== Number(id)); return current?.score == null || other?.score == null || current.score === other.score ? "—" : current.score > other.score ? "W" : "L"; });
  const renderPlayer = (player: Player, substitute = false, fallbackRole?: string) => { const position = substitute ? (playerRole(player) || fallbackRole) : fallbackRole || playerRole(player); const playerName = player.nickname ?? player.name ?? "Unknown player"; const icon = roleIcon(position); return <div className={`roster-player${substitute ? " substitute-player" : ""}`} key={player.id ?? playerName}>{player.image_url ? <img src={player.image_url} alt="" /> : <i>{playerName.slice(0, 1)}</i>}<span><b title={playerName}>{playerName}</b>{icon ? <img className="role-icon" src={icon} alt={`${position} role`} title={position ?? "Role"} /> : null}</span>{substitute && <small className="substitute-label">SUB</small>}</div>; };
  ...
  return <main className="competition-page"><Link href="/">← Back to matches</Link><div className="team-heading">{teamImage ? <img src={teamImage} alt="" /> : <i>{(teamName ?? "T").slice(0, 1)}</i>}<h1>{teamName ?? `Team ${id}`}</h1>{followControl}</div><section className="team-profile"><div><small>RECENT FORM</small><div className="large-form">{form.map((result, index) => <i className={result === "W" ? "win" : "loss"} key={index}>{result}</i>)}{!form.length && <span>No completed matches yet.</span>}</div></div></section><section className="competition-section"><h2>{rosterVerified ? "Main roster" : "Active roster"}</h2>{!rosterVerified && <p className="roster-warning">The source does not identify a verified starting five or substitutes, so all active players are shown without guessing.</p>}{displayedRoster.length ? <div className="roster">{displayedRoster.map((player) => renderPlayer(player, false, canonicalRole(playerRole(player))))}</div> : <p className="empty">The roster is not available yet.</p>}{substitutes.length ? <><h2 className="sub-roster-heading">Substitutes</h2><div className="roster">{substitutes.map((player) => renderPlayer(player, true))}</div></> : null}</section><section className="competition-section"><h2>Upcoming games</h2>{upcoming.length ? <div className="history-list">{upcoming.map((match) => <div className="history-row upcoming-row" key={match.id}><span className="history-date"><b>{displayDate(match.begin_at)}</b><small>{displayTime(match.begin_at)}</small>{match.tournament?.id ? <a className="competition-label" href={`/competition/${match.tournament.id}`}>{match.tournament.image_url || match.league?.image_url ? <img src={match.tournament.image_url ?? match.league?.image_url ?? ""} alt="" /> : null}{match.league?.name ?? match.tournament.name ?? "Competition"}</a> : <small className="competition-label">{match.league?.name ?? match.tournament?.name ?? ""}</small>}</span><span className="history-teams">{(match.opponents ?? []).map((entry) => <a href={entry.opponent?.id != null ? `/teams/${entry.opponent.id}` : undefined} key={entry.opponent?.id ?? entry.opponent?.name}>{entry.opponent?.image_url ? <img src={entry.opponent.image_url} alt="" /> : <i>{(entry.opponent?.name ?? "T").slice(0, 1)}</i>}{entry.opponent?.name ?? "TBD"}</a>)}</span><b className={match.status === "running" ? "live-label" : ""}>{match.status === "running" ? "LIVE" : "VS"}</b></div>)}</div> : <p className="empty">No upcoming games are scheduled for this team yet.</p>}</section><section className="competition-section"><h2>Last games played</h2>{recent.length ? <div className="history-list">{recent.map((match) => { const results = match.results ?? []; const current = results.find((result) => result.team_id === Number(id)); const won = current && results.some((result) => result.team_id !== Number(id) && (current.score ?? 0) > (result.score ?? 0)); return <div className="history-row" key={match.id}><span className="history-date"><b>{displayDate(match.begin_at)}</b><small className="competition-label">{match.league?.image_url ? <img src={match.league.image_url} alt="" /> : null}{match.league?.name ?? match.tournament?.name ?? ""}</small></span><span className="history-teams">{(match.opponents ?? []).map((entry) => { const opponentId = entry.opponent?.id; const opponentResult = results.find((result) => result.team_id === opponentId); const opponentWon = opponentResult?.score != null && results.some(result => result.team_id !== opponentId && result.score != null && opponentResult.score! > result.score); return <a className={opponentWon ? "winner" : ""} href={opponentId != null ? `/teams/${opponentId}` : undefined} key={opponentId}>{entry.opponent?.image_url ? <img src={entry.opponent.image_url} alt="" /> : <i>{(entry.opponent?.name ?? "T").slice(0, 1)}</i>}{entry.opponent?.name ?? "TBD"}</a>; })}</span><b className={won ? "winner" : ""}>{(match.opponents ?? []).map(entry => results.find(result => entry.opponent?.id != null && result.team_id === entry.opponent.id)?.score ?? "—").join(" – ")}</b></div>; })}</div> : <p className="empty">No completed games found for this team yet.</p>}</section></main>;
}
