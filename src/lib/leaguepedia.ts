import { competitionMatchScore } from "@/lib/data-quality";
...
export type LeaguepediaCompetition = { overviewPage: string; standardName?: string; standings: LeaguepediaStanding[]; matches: LeaguepediaMatch[]; source: "Leaguepedia"; confidence: "verified" };
  ...
  try {
    ...
    for (const name of candidates) {
      ...
      for (const row of rows) {
        const score = Math.max(...[row.OverviewPage, row.StandardName, row.Name].map((label) => competitionMatchScore(name, String(label ?? ""))));
        if (score > bestScore) { bestScore = score; tournament = row; }
      }
    }
    if (!tournament?.OverviewPage || bestScore < 90) return null;
    ...
    return { overviewPage, standardName: tournament.StandardName ? String(tournament.StandardName) : undefined, standings: standingsRows as LeaguepediaStanding[], matches: matchesRows as LeaguepediaMatch[], source: "Leaguepedia", confidence: "verified" } satisfies LeaguepediaCompetition;
  } catch { return null; }
}
